...
<?php
	...
	function modifyPro($dbConn){
		...
		$txtMachine = mysqli_real_escape_string($dbConn,$_POST['txtMachine']);
		$txtPrintingSize = $_POST['txtPrintingSize'];
		...
		$txtRetailPriceOutDoor = mysqli_real_escape_string($dbConn,$_POST['txtRetailPriceOutDoor']);
		$txtWholePriceOutDoor = mysqli_real_escape_string($dbConn,$_POST['txtWholePriceOutDoor']);
		$txtCostPriceOutDoor = mysqli_real_escape_string($dbConn,$_POST['txtCostPriceOutDoor']);
		$txtDesignOutdoor = mysqli_real_escape_string($dbConn,$_POST['txtDesignOutdoor']);
		$txtDesignPromotional = mysqli_real_escape_string($dbConn,$_POST['txtDesignPromotional']);
		...
		if(dbNumRows($result) > 0){
			...
			pro_print_sheet = '$txtPrintSheet', pro_item_per_sheet = '$txtItemPrinted',pro_machine = '$txtMachine',pro_printing_size = '$txtPrintingSize', pro_length = '$txtLength', pro_width = '$txtWidth', pro_min_qty = '$txtMinqty', pro_multi_qty = '$txtMultiqty',
			pro_s_item_cod = '$txtItemCod', pro_s_qty = '$txtQty', pro_s_qty_rim = '$txtQtyRem',";
			if($txtCat == 3){
				$sql2 .= " pro_s_cost_price = '$txtCostPriceOutDoor', pro_s_whole_price = '$txtWholePriceOutDoor', pro_s_retail_price ='$txtRetailPriceOutDoor',design_cost = '$txtDesignOutdoor' WHERE pro_id = '$hidId'";
			}else{
				$sql2 .= " pro_s_cost_price = '$txtCostPrice', pro_s_whole_price = '$txtWholePrice', pro_s_retail_price ='$txtRetailPrice',design_cost='$txtDesignPromotional'  WHERE pro_id = '$hidId'";
			}
			$result2 = dbQuery($dbConn, $sql2);
			$_SESSION['count'] = 0;
			$_SESSION['data'] = "success";
	...
	function addPro($dbConn){
		...
		if(dbNumRows($result) > 0){
			...
			$sql2 = "INSERT INTO tbl_product(cat_id, sub_cat_id, pro_title, pro_desc, pro_main_img, pro_img_1, pro_img_2, pro_img_3, pro_img_4, pro_price, pro_print_sheet, pro_item_per_sheet, pro_machine, pro_printing_size, pro_length, pro_width, pro_min_qty, pro_multi_qty, pro_simple,pro_s_item_cod, pro_s_qty, pro_s_qty_rim, pro_s_cost_price, pro_s_whole_price, pro_s_retail_price,design_cost) VALUES
			( '$txtCat', '$txtsubCat','$txtTitle', '$txtdesc', '$main_img','$img_1', '$img_2', '$img_3','$img_4', '$txtPrice','$txtPrintSheet', '$txtItemPrinted', '$txtMachine','$txtPrintingSize', '$txtLength','$txtWidth', '$txtMinqty','$txtMultiqty', '$txtfeature', '$txtItemCod', '$txtQty', '$txtQtyRem',";
			if($txtCat == 3){
				$sql2 .= "'$txtCostPriceOutDoor', '$txtWholePriceOutDoor', '$txtRetailPriceOutDoor','$txtDesignOutdoor')";
			}else{
				$sql2 .= "'$txtCostPrice', '$txtWholePrice', '$txtRetailPrice','$txtDesignPromotional')";
			}
			$result2 = dbQuery($dbConn, $sql2);
			$_SESSION['count'] = 0;
			$_SESSION['data'] = "success";
