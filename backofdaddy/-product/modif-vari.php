...
?>
	...
	<section class="content-header top_heading">
		<h1>Modify Product Variation</h1>
	</section>
	<!-- Main content -->
	<section class="content" >
		<!-- start any work here. -->
		<form name="frmAddMainNav" id="frmAddMainNav" method="post" action="processpro.php?action=modifprovar"  enctype="multipart/form-data" onsubmit="return validate(this)">
			<input type="hidden" value="<?php echo $PI; ?>" name="txtProId" />
			<div class="container-fluid container_block">
				<div class="row inner_heading">
					<h1>Modify Product Variation Details</h1><hr>
				</div>
				<div class="row">
					<div class="col-md-4">
						<div class="allstep" >
											...
											<tbody>
												...
												if(dbNumRows($result) >0){
													while($row = dbFetchAssoc($result)){
														...
														<tr>
															<td><input type="checkbox" id="var_id<?php echo $var_id; ?>" name="variation<?php echo $var_id; ?>"  value="1" <?php chkaddvariation($dbConn, $PI,$VarTypeId, $var_id); ?> /></td>		
															<td><?php echo $var_title;?></td>
															<td><input type="text" class="formField" id="cost_price<?php echo $var_id;?>" value="<?php chkcostmodvariprice($dbConn, $PI,$VarTypeId, $var_id); ?>" name="cost_price<?php echo $var_id; ?>" onchange="pricedouble<?php echo $var_id; ?>()" /></td>		
															<td><input type="text" class="formField" id="txtWholesaleP<?php echo $var_id; ?>" value="<?php chkwholesalemodvariprice($dbConn, $PI,$VarTypeId, $var_id); ?>" name="wholesale_price<?php echo $var_id; ?>" /></td>
															<td><input type="text" class="formField" id="txtRetailp<?php echo $var_id; ?>" value="<?php chkretailmodvariprice($dbConn, $PI,$VarTypeId, $var_id); ?>" name="retail_price<?php echo $var_id; ?>" /></td>
															<td>
																<?php  
																	if ($var_img != '') { ?>
