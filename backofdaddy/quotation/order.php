...
<table width='100%'>
    ...
    <tr>
		...
		foreach($objItems->items as $item) {
			$html .= "<tr>";
			$html .= "<td>".++$i."</td ><td>".$item->title."</td ><td >".productNamePr($dbConn, $item->productID)."</td ><td >";
			if ($item->catID == 3) {
...
if(isset($_POST['order'])){
	...
	if($objItems->items) {
		...
		$sql = "INSERT INTO `order`(order_id,title, client_id, user_id, order_note, order_delivery_time, status_id, department_id,advance,status)
 			VALUES ('$order_id1' ,'$title','$client_id','$user_id','$order_note','$order_delivery_time','$status_id','$department_id','$advance','$orderStatus')";
		...
		if($order_id){
			foreach($objItems->items as $item){
                $total1 += $item->total;
				$insertOrderDetail = "INSERT INTO `order_detail`(`order_detail_id`, `title`, `product_id`, `base_price`, `qty`, `cost_price`, `whole_sale_price`, `retail_price`, `height`, `width`, `rate`, `order_detail_status`, `order_id`)
 							  VALUES (Null,'$item->title','$item->productID','$item->basePrice','$item->qty','$item->costPrice','$item->wholesalePrice','$item->total','$item->length','$item->width','$item->rate','207','$order_id')";
...
//						die($insertOrderVariation);
		...
		/* 
			Custom total 
		*/
		$grandTotal=$_POST['grand_total'] +$incomeTax - $_POST['discount']-$_POST['advance'] ;
		$updateTotal="UPDATE `order` set total_amount='".$grandTotal."' WHERE order_id='".$order_id."' ";
