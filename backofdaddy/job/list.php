...
if(isset($_GET['jId'])){
	$id = $_GET['jId'];
	$sql =	"SELECT * FROM `job_order` WHERE Job_id = $id";
	$result     = dbQuery($dbConn,$sql);
}else{
	$rowsPerPage = 20;
	$sql		=	"SELECT * FROM `job_order` WHERE status = 0";
		...
		<div class="container-fluid container_block">
			<div>
				...
				if(isset($_SESSION['error'])){ ?>
						<div class="space"></div>
						<p class="alert alert-success"><?php echo $_SESSION['error'];  ?></p> <?php
						unset($_SESSION['error']);
				} ?>
			</div>
			<div class="row">
				<div class="col-sm-12">
					<a class="btn btn-danger pull-right" style="color: #ffffff;margin-top: 10px;" href="index.php?view=comp">Completed Job</a>
				</div>
			</div>
						...
						<tbody>
							...
							if(dbNumRows($result) > 0){
								...
								while($row = dbFetchAssoc($result)){
									...
									?>
									<tr>
										<td><a href="index.php?view=add&detId=<?=$order_detail_id?>&jobId=<?=$Job_id?>"><?=$i++;?></a></td>
										...
										<td>
											<?php if(modPerView($dbConn,"19")){?>
													<a href="processOrder.php?action=comp&jobId=<?=$Job_id?>" title="Complete"><i class="fa"></i> -- </a>&nbsp;
